...
class EntriesController < ApplicationController
  ...
  def surprise
    @surprise_entries = Entry.order('random()').limit(3).where("list_id = '1' AND medium_id IN \('7', '8', '9', '11', '12'\) AND url != ''") #We should use a variable to find the list and medium rather than use the ID
